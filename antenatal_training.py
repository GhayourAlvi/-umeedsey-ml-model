"""
Antenatal Complications Model Training Script

This script trains antenatal complications prediction models based on gestational age:
...
def load_and_preprocess_data():
    """
    Load and preprocess the dataset for antenatal training.

    Returns:
        tuple: (features_df, target_df, df_history, df_condition, ga_series, original_features_df)
    """
    ...
    # Extract gestational age series for trimester splitting
    ...
    # Extract only antenatal target variable
...
def train_antenatal_models(
    features_df, target_df, df_history, df_condition, ga_series, original_features_df
):
    """
    Train antenatal complications prediction models based on gestational age.
    Combined Trimester 1+2 and Trimester 3 each have 2 models: history-based and condition-based.

    Args:
        features_df: Features DataFrame
        target_df: Target variables DataFrame (antenatal complications)
    ...
    """
    print("Training antenatal models by trimester groups...")
    ...
    for trimester_key, trimester_name, min_weeks, max_weeks in trimester_groups:
        ...
        trimester_features_complete = original_features_df.loc[
            trimester_indices
        ].reset_index(drop=True)
        trimester_target_for_csv = target_df.loc[trimester_indices].reset_index(
            drop=True
        )

        # Combine features + target labels (gestational age is already in features in its original position)
        trimester_complete_data = pd.concat(
            [trimester_features_complete, trimester_target_for_csv], axis=1
        )
        ...
        print(
            f"✅ Saved complete trimester data: {csv_filename} ({len(trimester_complete_data)} rows)"
        )
        print(
            f"   Columns: {len(trimester_complete_data.columns)} (including Gestational Age in original position)"
        )

        # Loop through each target column (should be just one for antenatal)
        for i, target_col in enumerate(target_columns):
            ...
            # Remove rare labels that have fewer than 3 occurrences
            label_counts = target_encoded.sum(axis=0)
            rare_labels = label_counts[label_counts < 3].index.tolist()
            if rare_labels:
                print(
                    f"⚠️ Removing rare labels (count<3) for {trimester_key}: {rare_labels}"
                )
                target_encoded = target_encoded.drop(columns=rare_labels)

            # If no labels remain after filtering, skip training for this target
            if target_encoded.shape[1] == 0:
                print(
                    f"⚠️ No valid target labels remain after filtering for {trimester_key}. Skipping..."
                )
                continue
            ...
            # Remove RH incompatibility related columns from saved test data
            rh_cols = [
                c
                for c in merged_test_data.columns
                if ("rh incompat" in c.lower()) or ("rh incompatibility" in c.lower())
            ]
            if rh_cols:
                merged_test_data = merged_test_data.drop(columns=rh_cols)
                print(f"ℹ️ Removed RH columns from test data: {rh_cols}")

            merged_filename = (
                f"{target_col}_{trimester_key}_history_merged_test_data.csv"
            )
            ...
            # Also save the corresponding training rows + labels for reproducibility
            original_train_rows = trimester_history.iloc[train_idx].reset_index(
                drop=True
            )
            ...
            # Remove RH incompatibility related columns from saved train data
            rh_cols_train = [
                c
                for c in merged_train_data.columns
                if ("rh incompat" in c.lower()) or ("rh incompatibility" in c.lower())
            ]
            if rh_cols_train:
                merged_train_data = merged_train_data.drop(columns=rh_cols_train)
                print(f"ℹ️ Removed RH columns from train data: {rh_cols_train}")

            merged_train_filename = (
                f"{target_col}_{trimester_key}_history_merged_train_data.csv"
            )
            merged_train_data.to_csv(
                Testing_Data_Report + "/" + merged_train_filename, index=False
            )
            print(f"✅ Saved merged train data: {merged_train_filename}")
            ...
            pd.DataFrame(report_dict).transpose().to_csv(
                PRF_Report
                + "/"
                + f"antenatal_{trimester_key}_history_classification_report.csv"
            )
            ...
            original_test_rows = trimester_condition.iloc[test_idx].reset_index(
                drop=True
            )
            original_test_labels = target_encoded.iloc[test_idx].reset_index(drop=True)
            merged_test_data = pd.concat(
            ...
            merged_filename = (
                f"{target_col}_{trimester_key}_condition_merged_test_data.csv"
            )
            merged_test_data.to_csv(
                Testing_Data_Report + "/" + merged_filename, index=False
            )
            ...
            # Also save the corresponding training rows + labels for reproducibility
            original_train_rows = trimester_condition.iloc[train_idx].reset_index(
                drop=True
            )
            original_train_labels = target_encoded.iloc[train_idx].reset_index(
                drop=True
            ...
            merged_train_filename = (
                f"{target_col}_{trimester_key}_condition_merged_train_data.csv"
            )
            merged_train_data.to_csv(
                Testing_Data_Report + "/" + merged_train_filename, index=False
            )
            ...
            pd.DataFrame(report_dict).transpose().to_csv(
                PRF_Report
                + "/"
                + f"antenatal_{trimester_key}_condition_classification_report.csv"
            )

    return models_list, encoder_list, feature_mapping_list, col_list, trimester_info


def save_antenatal_models(
    models_list, encoder_list, feature_mapping_list, col_list, trimester_info
):
    """
    Save trained antenatal models and artifacts to save_models folder.
    ...
    """
    print("\nSaving antenatal models and artifacts...")
    ...
    model_names = {
        "trimester_1_2_history": "antenatal_trimester1_2_history.pkl",
        "trimester_1_2_condition": "antenatal_trimester1_2_condition.pkl",
        "trimester_3_history": "antenatal_trimester3_history.pkl",
        "trimester_3_condition": "antenatal_trimester3_condition.pkl",
    }

    for idx, trimester_key in enumerate(trimester_info):
        if idx < len(models_list):
            model_data = {
                "antenatal_model": models_list[idx],
                "col_list": (
                    col_list[idx] if isinstance(col_list[idx], list) else col_list[idx]
                ),
            ...
            model_filename = model_names.get(
                trimester_key, f"antenatal_{trimester_key}.pkl"
            )
    ...
    combined_path = os.path.join(save_dir, "antenatal_all_trimesters.pkl")
    joblib.dump(combined_model, combined_path)
    print(f"✅ Saved combined model file to {combined_path}")


def main():
    """Main training pipeline for antenatal models by trimester."""
    print("=" * 50)
    print("ANTENATAL COMPLICATIONS PREDICTION MODELS TRAINING (BY TRIMESTER)")
    print("=" * 50)
    ...
    try:
        ...
        print(
            f"  Combined Trimester 1+2 (0-189 days, 0-27 weeks): {trimester_masks['trimester_1_2'].sum()} samples"
        )
        print(
            f"  Trimester 3 (196+ days, 28+ weeks): {trimester_masks['trimester_3'].sum()} samples"
        )

        # Train antenatal models by trimester
        models_list, encoder_list, feature_mapping_list, col_list, trimester_info = (
            train_antenatal_models(
                features_df,
                target_df,
                df_history,
                df_condition,
        ...
        print("\n" + "=" * 50)
        print(f"ANTENATAL TRAINING COMPLETED SUCCESSFULLY!")
        print(
            f"Total models trained: {len(models_list)} (Combined Trimester 1+2: 2 models, Trimester 3: 2 models)"
        )
        print("=" * 50)
