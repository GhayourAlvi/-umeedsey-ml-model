"""
Mode of Delivery Model Training Script

This script trains mode of delivery prediction models based on gestational age:
...
def load_and_preprocess_data():
    """
    Load and preprocess the dataset for mode of delivery training.
    ...
    # Extract only mode of delivery target variable
...
def train_mode_of_delivery_models(features_df, target_df, df_history, df_condition, ga_series, original_features_df):
    """
    Train mode of delivery prediction models based on gestational age.
    Combined Trimester 1+2 and Trimester 3 each have 2 models: history-based and condition-based.

    Args:
        features_df: Features DataFrame
        target_df: Target variables DataFrame (mode of delivery)
    ...
    """
    print("Training mode of delivery models by trimester groups...")
    
    # Create a directory for trimester CSV files
    # Save all CSV files to Testing_Data_Report folder
    trimester_csv_dir = Testing_Data_Report
    ...
    for trimester_key, trimester_name, min_weeks, max_weeks in trimester_groups:
        ...
        # Loop through each target column (should be just one for mode of delivery)
        for i, target_col in enumerate(target_columns):
            ...
            pd.DataFrame(report_dict).transpose().to_csv(
                PRF_Report
                + "/"
                + f"mode_of_delivery_{trimester_key}_history_classification_report.csv"
            )

            # Train Condition-based model
            print(f"\nTraining condition-based model for {trimester_name}...")
            df_expanded, specific_col, specific_val = expand_comma_columns(trimester_condition)
            ...
            original_test_rows = trimester_condition.iloc[test_idx].reset_index(drop=True)
            original_test_labels = target_encoded.iloc[test_idx].reset_index(drop=True)
            merged_test_data = pd.concat([original_test_rows, original_test_labels], axis=1)
            ...
            pd.DataFrame(report_dict).transpose().to_csv(
                PRF_Report
                + "/"
                + f"mode_of_delivery_{trimester_key}_condition_classification_report.csv"
            )

    return models_list, encoder_list, feature_mapping_list, col_list, trimester_info


def save_mode_of_delivery_models(models_list, encoder_list, feature_mapping_list, col_list, trimester_info):
    """
    Save trained mode of delivery models and artifacts to save_models folder.
    ...
    """
    print("\nSaving mode of delivery models and artifacts...")
    ...
    for idx, trimester_key in enumerate(trimester_info):
        if idx < len(models_list):
            model_data = {
                "mode_of_delivery_model": models_list[idx],
            ...
            model_filename = model_names.get(trimester_key, f"mode_of_delivery_{trimester_key}.pkl")
    ...
    combined_path = os.path.join(save_dir, "mode_of_delivery_all_trimesters.pkl")
    joblib.dump(combined_model, combined_path)
    print(f"✅ Saved combined model file to {combined_path}")


def main():
    """Main training pipeline for mode of delivery models by trimester."""
    print("=" * 50)
    print("MODE OF DELIVERY PREDICTION MODELS TRAINING (BY TRIMESTER)")
    print("=" * 50)
    ...
    try:
        ...
        # Train mode of delivery models by trimester
        models_list, encoder_list, feature_mapping_list, col_list, trimester_info = train_mode_of_delivery_models(
            features_df,
            target_df,
            df_history,
            df_condition,
        ...
        print("\n" + "=" * 50)
        print(f"MODE OF DELIVERY TRAINING COMPLETED SUCCESSFULLY!")
