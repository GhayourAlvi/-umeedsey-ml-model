"""
Antenatal Complications Inference Script

This script loads trained antenatal models and makes predictions on CSV files.
...
Usage:
    python antenatal_inference.py input.csv output.csv
"""
...
def safe_process_field(
    field_name: str, field_value: Any, conditions: List[str]
) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            cond: ("yes" if cond.lower() in row_list else "no") for cond in conditions
        }
    except Exception:
...
def creating_features(
    data: List[str],
    record_lower: Dict[str, Any],
    encoder_model: Any,
    dic_json: Dict[str, List[str]],
):
    ...
    try:
        if (
            hasattr(encoder_model, "feature_names_in_")
            and encoder_model.feature_names_in_ is not None
        ):
    ...
    if expected_features is not None:
        ...
        feature_df = feature_df[
            [col for col in expanded_columns if col in feature_df.columns]
        ]
...
def get_top_k_labels(
    col_list: List[str], probs_list: List[float], k: int = 3
) -> List[str]:
...
def load_models():
    """Load all trained antenatal models."""
    models = {}
    ...
    for key, filename in model_files.items():
        ...
        if os.path.exists(filepath):
            try:
                ...
                models[key] = {
                    "model": model_data.get("antenatal_model"),
...
def preprocess_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "Per speculum examination (If Yes, specify findings)" not in record or pd.isna(
        record.get("Per speculum examination (If Yes, specify findings)")
    ):
        record["Per speculum examination (If Yes, specify findings)"] = "no"
    if "Per vaginal examination (If Yes, specify findings)" not in record or pd.isna(
        record.get("Per vaginal examination (If Yes, specify findings)")
    ):
...
def predict_antenatal(
    record: Dict[str, Any],
    models: Dict,
    history_data: List[str],
    condition_data: List[str],
) -> Dict[str, Any]:
    """Make antenatal prediction for a single record.
    ...
    try:
        ...
        history_top = get_top_k_labels(
            history_model_data["col_list"], probs_history, k=2
        )
        ...
        if probs_history and len(probs_history) > 0:
            ...
            top_probs = [
                float(probs_history[i]) for i in top_indices if i < len(probs_history)
            ]
            top_labels = [
                history_model_data["col_list"][i]
                for i in top_indices
                if i < len(history_model_data["col_list"])
            ]
            ...
            result["history_prediction"] = (
                ",".join(str(x) for x in history_top)
                if history_top
                else "Model failed to predict"
            )
    ...
    try:
        ...
        condition_top = get_top_k_labels(
            condition_model_data["col_list"], probs_condition, k=2
        )
        ...
        if probs_condition and len(probs_condition) > 0:
            ...
            top_indices = list(np.argsort(-arr)[:2])
            top_probs = [
                float(probs_condition[i])
                for i in top_indices
                if i < len(probs_condition)
            ]
            top_labels = [
                condition_model_data["col_list"][i]
                for i in top_indices
                if i < len(condition_model_data["col_list"])
            ]

            result["condition_prediction"] = ",".join(str(x) for x in top_labels)
            result["condition_probability"] = ",".join([f"{p:.4f}" for p in top_probs])
        else:
            result["condition_prediction"] = (
                ",".join(str(x) for x in condition_top)
                if condition_top
                else "Model failed to predict"
            )
    ...
    # Combine predictions (union of both)
    history_labels = (
        result["history_prediction"].split(",") if result["history_prediction"] else []
    )
    condition_labels = (
        result["condition_prediction"].split(",")
        if result["condition_prediction"]
        else []
    )
...
def main():
    ...
    input_csv = ".\dataset\Data for AI prediction 23rd Jan.csv"
    output_csv = "output_94_antenatal1.csv"
    ...
    print("=" * 60)
    print("ANTENATAL INFERENCE SCRIPT")
    print("=" * 60)
    ...
    if all(v is None for v in models.values()):
        print(
            "❌ Error: No models loaded. Please ensure model files exist in save_models/"
        )
        sys.exit(1)
    ...
    if not history_data or not condition_data:
        print(
            "❌ Error: Could not load feature lists from dataset/Divided History Features.csv"
        )
        sys.exit(1)
    ...
    for idx, row in df.iterrows():
        ...
        record = row.to_dict()
        try:
            result = predict_antenatal(record, models, history_data, condition_data)
    ...
    df["Antenatal_History_Model_Prediction_Top2"] = history_predictions
    df["Antenatal_History_Model_Probability_Top2"] = history_probabilities
    df["Antenatal_Condition_Model_Prediction_Top2"] = condition_predictions
    df["Antenatal_Condition_Model_Probability_Top2"] = condition_probabilities
    df["Antenatal_Combined_Prediction"] = combined_predictions
    ...
    try:
        ...
        print(f"\nOutput columns:")
        print(
            f"  - Antenatal_History_Model_Prediction_Top2: Top 2 predictions from history model"
        )
        print(
            f"  - Antenatal_History_Model_Probability_Top2: Probabilities for top 2 history predictions"
        )
        print(
            f"  - Antenatal_Condition_Model_Prediction_Top2: Top 2 predictions from condition model"
        )
        print(
            f"  - Antenatal_Condition_Model_Probability_Top2: Probabilities for top 2 condition predictions"
        )
        print(
            f"  - Antenatal_Combined_Prediction: Combined predictions from both models"
        )
        print(
            f"  - Trimester_Group_Used: Trimester group (trimester_1_2 or trimester_3) used for prediction"
        )
    except Exception as e:
