"""
Neonatal Inference Script

This script loads trained neonatal models and makes predictions on CSV files.
...
Usage:
    python neonatal_inference.py input.csv output.csv
"""
...
def load_models():
    """Load all trained neonatal models."""
    models = {}
    ...
    for key, filename in model_files.items():
        ...
        if os.path.exists(filepath):
            try:
                ...
                models[key] = {
                    "model": model_data.get("neonatal_model"),
...
def predict_neonatal(record: Dict[str, Any], models: Dict, history_data: List[str], condition_data: List[str]) -> Dict[str, Any]:
    """Make neonatal prediction for a single record.
    ...
    # Create features for condition model (with Apgar scores)
    condition_data_with_apgar = condition_data.copy()
    condition_data_with_apgar.extend(
        ["Neonatal Apgar Score:(1 Minute)", "Neonatal Apgar Score:(5 Minute)"]
    )
    
    try:
        neonatal_condition_features = creating_features(
            condition_data_with_apgar,
            temp_record,
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python neonatal_inference.py <input_csv> <output_csv>")
        print("Example: python neonatal_inference.py input.csv output.csv")
        sys.exit(1)
    ...
    print("=" * 60)
    print("NEONATAL INFERENCE SCRIPT")
    print("=" * 60)
    ...
    for idx, row in df.iterrows():
        ...
        record = row.to_dict()
        try:
            result = predict_neonatal(record, models, history_data, condition_data)
    ...
    # Add predictions to dataframe - separate columns for each model
    df["Neonatal_History_Model_Prediction_Top2"] = history_predictions
    df["Neonatal_History_Model_Probability_Top2"] = history_probabilities
    df["Neonatal_Condition_Model_Prediction_Top2"] = condition_predictions
    df["Neonatal_Condition_Model_Probability_Top2"] = condition_probabilities
    df["Neonatal_Combined_Prediction"] = combined_predictions
    ...
    try:
        ...
        print(f"\nOutput columns:")
        print(f"  - Neonatal_History_Model_Prediction_Top2: Top 2 predictions from history model")
        print(f"  - Neonatal_History_Model_Probability_Top2: Probabilities for top 2 history predictions")
        print(f"  - Neonatal_Condition_Model_Prediction_Top2: Top 2 predictions from condition model")
        print(f"  - Neonatal_Condition_Model_Probability_Top2: Probabilities for top 2 condition predictions")
        print(f"  - Neonatal_Combined_Prediction: Combined predictions from both models")
